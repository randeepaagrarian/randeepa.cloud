                ...
                if (err) {
                    ...
                    if (sendingMessage) {
                        ...
                        async.eachSeries(textMessageNumbers, function (number, callback) {
                            request('https://richcommunication.dialog.lk/api/sms/inline/send.php?destination=' + number + '&q=' + args.textAPIKey + '&message=' + textMessage, { json: true }, function (err, res, body) {
                                if (err) {
                                    oneOrMoreFails = true
